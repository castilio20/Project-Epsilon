package com.vulp.druidcraft.entity.client.render;
...
public class CustomBoatModel extends EntityRenderer<CustomBoatEntity> {
    ...
    public CustomBoatModel(EntityRendererProvider.Context context, boolean chest) {
        ...
        this.boatResources = Stream.of(CustomBoatEntity.Type.values()).collect(ImmutableMap.toImmutableMap(type -> type, type -> Pair.of(new ResourceLocation(Druidcraft.MODID, getTextureLocation(type, chest)), this.createBoatModel(context, type, chest))));
    }
    ...
    private static ModelLayerLocation createLocation(String path) {
        return new ModelLayerLocation(new ResourceLocation(Druidcraft.MODID, path), "main");
    }
